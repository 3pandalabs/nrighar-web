...
export async function applicationRoutes(app: FastifyInstance) {
  ...
  // Async message thread between the owner and the applicant on one
  // application — either side may read/post; not tenant- or owner-only,
  // since both roles need this. Authorization (must be a participant on
  // this specific application) happens in the activity, not here.
  app.get("/applications/:id/messages", { preHandler: requireAuth }, async (req, reply) => {
    const { id } = req.params as { id: string };
    return sendWorkflow(reply, "listApplicationMessagesWorkflow", [{ applicationId: id, userId: req.userId! }]);
  });

  app.post(
    "/applications/:id/messages",
    { preHandler: requireAuth, schema: { body: messageBody } },
    async (req, reply) => {
      const { id } = req.params as { id: string };
      const { body } = req.body as z.infer<typeof messageBody>;
      return sendWorkflow(
        reply,
        "sendApplicationMessageWorkflow",
        [{ applicationId: id, userId: req.userId!, body }],
        201,
      );
    },
  );
